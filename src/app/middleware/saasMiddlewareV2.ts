...
const getTenant = async (host: string | undefined, connection: any) => {
  ...
  const tenant = isLocalHostAndNoConnectedTenant(host ?? '') ? await tenantModel.findOne({ origin: { $regex: 'localhost' } }) : await tenantModel.findOne({ origin: host });
...
const saas = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    const domain = hasConnectedTenant ? `${config.protocol}${config.connectTenantOrigin}.${config.mainDomain}` : req.headers.origin;
